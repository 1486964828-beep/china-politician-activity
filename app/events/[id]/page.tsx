    ...
    <main className="mx-auto max-w-5xl px-4 py-8 md:px-8">
      <div className="rounded-[2rem] border border-line bg-white/90 p-6 shadow-soft">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <Badge>{event.eventDate.toISOString().slice(0, 10)}</Badge>
        </div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {event.leaders.map((item) => (
                  <Badge key={item.id} tone="amber">
                    {item.leader.name} / {roleTypeLabels[item.leader.roleType]}
                  </Badge>
                ))}
              </div>
            </section>
          ...
          {event.sources.map((source) => (
            <div key={source.id} className="rounded-3xl border border-slate-100 p-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge tone="amber">{credibilityLabels[source.rawArticle.credibilityLevel ?? "D"]}</Badge>
              ...
              <p className="mt-2 text-sm text-slate-500">
                发布时间：{source.rawArticle.publishTime ? source.rawArticle.publishTime.toISOString().slice(0, 10) : "未识别"}
              </p>
